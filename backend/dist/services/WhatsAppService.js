...
class WhatsAppService {
            ...
            for (const msg of m.messages) {
                ...
                if (msg.message.imageMessage || (msg.message.documentMessage && msg.message.documentMessage.mimetype?.startsWith('image/'))) {
                    try {
                        ...
                        const stream = await downloadContentFromMessage(msg.message.imageMessage || msg.message.documentMessage, messageType);
                ...
                if (!text && (msg.message.audioMessage)) {
                    try {
                        ...
                        fs_1.default.mkdirSync(path.join(process.cwd(), 'uploads'), { recursive: true });
                ...
                if (!this.chatHistories.has(groupJid))
                    this.chatHistories.set(groupJid, []);
                ...
                result.sharedParticipants.forEach((p) => allUserIds.add(p.telegramUserId));
                result.personalExpenses.forEach((p) => allUserIds.add(p.telegramUserId));
    ...
    static async loadSettingsFromDb() {
        try {
            let setting = await Setting_1.Setting.findOne({ key: 'whatsappNotifications' });
            if (setting) {
                ...
                await Setting_1.Setting.create({ key: 'whatsappNotifications', value: 'true' });
            }
        }
        catch (e) {
    ...
    static async setNotificationsEnabled(enabled) {
        ...
        await Setting_1.Setting.findOneAndUpdate({ key: 'whatsappNotifications' }, { value: enabled ? 'true' : 'false' }, { upsert: true });
    }
    static getIsReady() {
        return this.isReady;
    }
    ...
    static async sendGroupMessage(groupTitle, message, _quoteMessageId) {
    ...
    static async sendGroupPoll(groupTitle, pollQuestion, options, settlementId) {
        if (!this.isReady || !this.sock) {
            console.log('⚠️ WhatsApp client is not ready. Message not sent.');
            return null;
        }
    ...
    static async broadcastToSplitHistory(item, type) {
        try {
            const Group = require('../models/Group').default;
            const User = require('../models/User').default;
            ...
            if (!splitHistoryGroup && !splitHistoryJid)
                return;
            const allUserIds = new Set();
            if (type === 'expense') {
                allUserIds.add(item.paidByTelegramUserId);
                item.sharedParticipants.forEach((p) => allUserIds.add(p.telegramUserId));
                item.personalExpenses.forEach((p) => allUserIds.add(p.telegramUserId));
            }
            else {
                allUserIds.add(item.paidByTelegramUserId);
            ...
            try {
                ...
                allUsers.forEach((u) => globalUserMap.set(u.telegramUserId, u.firstName || u.username || u.telegramUserId.toString()));
                let hasBalances = false;
